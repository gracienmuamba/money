...
export async function asKedDecrimentpret(prix, fraisAgent, money, getPhone, getUser, main, cdf) {
   ...
   let send = { date: moment().format('LLL'), solde: `${money} CDF`, phone: getPhone, user: getUser, type: 'envoyer', price: parseInt(Number(main)), actual: parseInt(Number(cdf)) - Number(prix) + ' CDF' + ' [DÉPÔT PRÊT]', unite: 'CDF' }
